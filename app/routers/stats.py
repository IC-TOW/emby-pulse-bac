...
def get_clean_name(item_name, item_type):
    ...
    # 如果找到了，统一格式化为 "剧名 - 第 X 季"
    if season_num is not None:
        return f"{series_name} - 第 {season_num} 季"

    # 兜底：如果名字全连在一起 (如 破事精英S02E01)
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for item in res.json():
                ...
                data.append({
                    "Id": item.get("Id"), "Name": item.get("Name"), "SeriesName": item.get("SeriesName", ""), 
                    "Year": item.get("ProductionYear"), "Rating": item.get("CommunityRating"), 
                    "Type": item.get("Type"), "DateCreated": item.get("DateCreated")
                })
            return {"status": "success", "data": data}
    except: pass
    return {"status": "error", "data": []}
...
def api_top_movies(user_id: Optional[str] = None, category: str = 'all', sort_by: str = 'count'):
    try:
        ...
        top_50 = res[:50]
        resolve_poster_ids(top_50) # 🔥 触发海报ID自动升级
        ...
        try:
            ...
            for r in raw_fav:
                ...
                if clean not in agg_fav: 
                    agg_fav[clean] = {"ItemName": clean, "ItemId": row_dict["ItemId"], "c": 0, "d": 0}
                agg_fav[clean]["c"] += 1
            ...
            if top_fav:
                resolve_poster_ids([top_fav]) # 🔥 获取最爱影片的高清季封面

        except Exception as e: print(f"Fav Error: {e}")
...
def api_poster_data(user_id: Optional[str] = None, period: str = 'all'):
    try:
        ...
        return {"status": "error", "data": {"plays": 0, "hours": 0}}
